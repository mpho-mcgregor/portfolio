...
bookingsRouter.get('/', requireAuth, (req, res) => {
  const rows = db
    .prepare(
      `SELECT b.id, b.date, b.name, b.created_at AS createdAt,
...
bookingsRouter.post('/', requireAuth, (req, res) => {
  ...
  db.prepare(
    `INSERT INTO bookings (id, user_id, creative_id, service_id, name, date, created_at)
     VALUES (@id, @user_id, @creative_id, @service_id, @name, @date, @created_at)`
  ).run(booking);

  res.status(201).json({ id: booking.id, date: booking.date, name: booking.name });
});
